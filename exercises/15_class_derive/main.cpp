...
struct X {
    ...
    X(int x_) : x(x_) { std::cout << "X(" << x << ')' << std::endl; }
...
struct A {
    ...
    A(int a_) : a(a_) { std::cout << "A(" << a << ')' << std::endl; }
...
struct B : public A {
    ...
    B(B const& other) : A(other.a), x(other.x) {
        std::cout << "B(B const &) : A(" << a << "), x(X(" << x.x << "))"
                  << std::endl;
    }
    ~B() { std::cout << "~B(" << a << ", X(" << x.x << "))" << std::endl; }
};
